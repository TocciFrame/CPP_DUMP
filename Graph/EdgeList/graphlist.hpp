...
class GraphList : public Graph {
    ...
    // --- Helper for Kruskal: Find Set (with path compression) ---
    ...
    void kruskal() {
        ...
        // Create a copy of the edges array to sort
        Edge** sortedEdges = (Edge**) malloc(num_edge * sizeof(Edge*));
        for (int i = 0; i < num_edge; i++) {
            sortedEdges[i] = s_edges[i];
        }
        // Sort edges based on their names (weights) using Bubble Sort
        for (int i = 0; i < num_edge - 1; i++) {
            for (int j = 0; j < num_edge - i - 1; j++) {
                if (sortedEdges[j]->getName() > sortedEdges[j + 1]->getName()) {
        ...
        // 'parent' array stores the set leader for each vertex index
        int parent[10];
        for (int i = 0; i < num_vert; i++) {
        ...
        for (int i = 0; i < num_edge; i++) {
            ...
            char u = sortedEdges[i]->getOrig();
            char v = sortedEdges[i]->getDest();
            int w = sortedEdges[i]->getName();

            int uIdx = getVertInd(u);
            int vIdx = getVertInd(v);

            // Find roots of the sets
            int setU = findSet(parent, uIdx);
            int setV = findSet(parent, vIdx);

            // If they are in different sets, add edge (no cycle)
            if (setU != setV) {
        ...
        cout << "--------------------" << endl;
        cout << "Total MST Cost: " << mstWeight << endl;
